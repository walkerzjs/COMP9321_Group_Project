...
# draw correlation graph from the combined data set
@mod.route('/view/similarity_analysis_new', methods=['GET', 'POST'])
def similarity_analysis_all_new():
    try:
        country = request.form['country']
        #        feature2 = request.form['pm_happy_feature2']
        year = request.form['year']
...
#    res_dict = json.loads(res_str)
    ...
    print(countryDict)
    print("country: ",country)
    return render_template("similarity_analysis_new.html", countries = countryDict,
                           year=year, country_input = country)
