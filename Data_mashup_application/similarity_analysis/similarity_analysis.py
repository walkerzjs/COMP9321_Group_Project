...
import json
mod = Blueprint('data_mashup_similarity', __name__)
...
def compute_similarity_all():
    ...
    data_json = {"country1Data": country1Data.content.decode(), "country1Pollution": country1Pollution.content.decode(), "allCountryData": allCountryData.content.decode(), "allCountryPollutionData": allCountryPollutionData.content.decode(),"year": year, "country":country1}
    url = "http://127.0.0.1:1234/api/similarity/all"
    accept_tp = "JSON"
    print("start send")
    print(type(country1Data))
    result = requests.post(url, json=data_json)
                
    res_str = result.content.decode()
    countryDict = json.loads(res_str)
    if result:
...
def country_similarity_all(year, country):
    country1 = country
    year = str(year)
...
#    year = request.args.get('year')
    ...
    allCountryData = requests.get(urlWorldHappiness + worldHappinessRouteAll + year)
    allCountryPollutionData = requests.get(urlPollution + pollutionRouteAll + year)
    data_json = {"country1Data": country1Data.content.decode(), "country1Pollution": country1Pollution.content.decode(), "allCountryData": allCountryData.content.decode(), "allCountryPollutionData": allCountryPollutionData.content.decode(),"year": year, "country":country}
    url = "http://127.0.0.1:1234/api/similarity/all"
    accept_tp = "JSON"
    print("start send")
    print(type(country1Data))
